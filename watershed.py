        ...
        if event == cv.CV_EVENT_LBUTTONUP or not (flags & cv.CV_EVENT_FLAG_LBUTTON):
            ...
            if self.prev_pt:
                for dst in self.dests:
                    cv.Line(dst, self.prev_pt, pt, cv.ScalarAll(255), 2, 8, 0)
            self.prev_pt = pt
...
def watershed_manuel(image_filtree):
    from cvnumpy import *
    global img
    # path = tkFileDialog.askopenfilename(filetypes = [("Fichiers Image","*.jpeg;*.jpg;*.png;*.bmp"),("All","*")]) 	
    img0 = cv.CloneImage(image_filtree) #cv.LoadImage( path )
    ...
    marker_background = cv.CloneImage(marker_mask)
    
    sk = Sketcher("image", [img, marker_mask, marker_background])
    
    while True:
        c = cv.WaitKey(0)
        if c == 27 or c == ord('q'):
            cv.DestroyWindow("image"); cv.DestroyWindow("watershed transform")
            return cv2array(markers)[:,:,0], wshed
        if c == ord('r'):
            cv.Zero(marker_mask)
            cv.Copy(img0, img)
            cv.ShowImage("image", img)
